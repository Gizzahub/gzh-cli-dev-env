...
lint-ci: install-golangci-lint ## run golangci-lint for CI
	...
	"$(GOLANGCI_LINT)" run -c .golangci.yml --output.text.colors=false

lint-count: install-golangci-lint ## count total lint issues without fixing
	@echo -e "$(CYAN)Counting lint issues...$(RESET)"
	@ISSUES=$$("$(GOLANGCI_LINT)" run -c .golangci.yml --max-issues-per-linter=0 --max-same-issues=0 2>/dev/null | grep -E "^[^[:space:]].*\\([^)]+\\)$$" | wc -l); \
...
lint-summary: install-golangci-lint ## show lint issues summary by linter
	...
	@"$(GOLANGCI_LINT)" run -c .golangci.yml --max-issues-per-linter=0 --max-same-issues=0 2>/dev/null | \
...
lint-status: install-golangci-lint ## comprehensive lint status report
	...
	@TOTAL=$$("$(GOLANGCI_LINT)" run -c .golangci.yml --max-issues-per-linter=0 --max-same-issues=0 2>/dev/null | grep -E "^[^[:space:]].*\\([^)]+\\)$$" | wc -l); \
	ERRORS=$$("$(GOLANGCI_LINT)" run -c .golangci.yml --max-issues-per-linter=0 --max-same-issues=0 --output.json.path=stdout 2>/dev/null | jq -r '.Issues[]? | select(.Severity=="error") | .Severity' 2>/dev/null | wc -l || echo "0"); \
	WARNINGS=$$("$(GOLANGCI_LINT)" run -c .golangci.yml --max-issues-per-linter=0 --max-same-issues=0 --output.json.path=stdout 2>/dev/null | jq -r '.Issues[]? | select(.Severity=="warning") | .Severity' 2>/dev/null | wc -l || echo "0"); \
...
lint-json: install-golangci-lint ## export lint results to JSON for further analysis
	...
	@"$(GOLANGCI_LINT)" run -c .golangci.yml --max-issues-per-linter=0 --max-same-issues=0 --output.json.path=lint-report.json 2>/dev/null || true
